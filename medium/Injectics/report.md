...
## 2. Attack Chain
```
Nmap → Gobuster → mail.log disclosure → SQLi auth bypass → DB table drop → Credential restoration → Admin login → Twig SSTI ({{7*7}}) → Sandbox enum → sort('passthru') bypass → RCE → Flag exfil
```
...
|---------|--------|-------|
| `{{["id"]|map("system")}}` | ❌ Closure error | `map` filter requires Closure in sandbox mode |
| `{{["id"]|filter("system")}}` | ❌ Closure error | `filter` also hardened |
| `{{["id"]|reduce("system")}}` | ❌ Closure error | `reduce` blocked |
| `{{_self}}` | ✅ Returns `__string_template__[hash]` | `_self` object accessible |
| `{{_context}}` | ✅ Returns `Array` | Context accessible but can't iterate |
| `{{ _context|keys }}` | ❌ Filter not allowed | Key enumeration blocked |
| `{{ source('/etc/passwd') }}` | ❌ Function not allowed | File read blocked |
...
- Always enumerate accessible objects (`_self`, `_context`) even when common filters are blocked
...
**Personal Takeaway:**
This was my first encounter with a *properly hardened* Twig sandbox. The `sort('passthru')` bypass reinforced that **enumeration beats assumption**. When `map`/`filter`/`reduce` all failed, I could have given up — but systematically testing less-common filters revealed the oversight. The patience to test edge cases is what separates a script kiddie from a penetration tester.
